...
public class AmapLocationService {
    ...
    private final AMapLocationListener locationListener = new AMapLocationListener() {
        ...
        public void onLocationChanged(AMapLocation location) {
            if (null != location) {
                SearLocalBean localBean = new SearLocalBean();
                StringBuffer sb = new StringBuffer();
                //errCode等于0代表定位成功，其他的为定位失败，具体的可以参照官网定位错误码说明
                if(location.getErrorCode() == 0){


                    localBean.setName(location.getPoiName());
                    localBean.setPoiId(null);
                    localBean.setCity(location.getCity());
                    localBean.setDistrict(location.getDistrict());
                    localBean.setLon(location.getLongitude());
                    localBean.setLat(location.getLatitude());

                    localBean.setAccuracy(location.getAccuracy());
...
//                sb.append("* GPS状态：").append(getGPSStatusString(location.getLocationQualityReport().getGPSStatus())).append("\n");
             ...
             //   localBean.setTxt(result);
                if(onLocationListener != null)
                    onLocationListener.backLocalLatLon(localBean);

               // Timber.e("------定位结果="+result);
    ...
    public interface OnLocationListener{
        void backLocalLatLon(SearLocalBean searLocalBean);
    }
}
