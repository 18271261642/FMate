...
public final class RequestHandler implements IRequestHandler {
    ...
    public Object requestSucceed(@NonNull HttpRequest<?> httpRequest, @NonNull Response response,
        ...
        if (!response.isSuccessful()) {
            // 返回响应异常
            throw new ResponseException("响应异常" + ", responseCode: " +
...
//                ActivityManager.getInstance().finishAllActivities();
                ...
                // 代表登录失效，需要重新登录
               // requestFail(httpRequest,new TokenException(mApplication.getString(R.string.http_token_error)));
                //throw new TokenException(mApplication.getString(R.string.http_token_error));
                throw new TokenException(model.getMessage());
            }

            if(model.isNo200Code()){

                return ((HttpData<?>) result).getMessage();
            }
...
//                ActivityManager.getInstance().finishActivity(HomeActivity.class);
        ...
        if (e instanceof SocketTimeoutException) {
            return new TimeoutException("超时", e);
        }
        ...
        if (e instanceof UnknownHostException) {
            ...
            // 没有连接就是网络异常
            return new NetworkException("网络异常", e);
        }

        if (e instanceof IOException) {
            return new CancelException("IO异常", e);
        }
    ...
    public Object readCache(@NonNull HttpRequest<?> httpRequest, @NonNull Type type, long cacheTime) {
        ...
        String cacheValue = Hawk.get(cacheKey, null);
    ...
    public boolean writeCache(@NonNull HttpRequest<?> httpRequest, @NonNull Response response, @NonNull Object result) {
        ...
        EasyLog.printJson(httpRequest, cacheValue);
        return Hawk.put(cacheKey, cacheValue);
    }
