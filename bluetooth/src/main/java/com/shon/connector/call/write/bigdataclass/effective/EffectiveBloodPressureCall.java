package com.shon.connector.call.write.bigdataclass.effective;
 ...
 * APP 端获取指定的温度数据
 */
public class EffectiveBloodPressureCall extends WriteCallback {

    BleWrite.EffectiveBloodPressureInterface mInterface;
    ...
    public EffectiveBloodPressureCall(String address, int position, BleWrite.EffectiveBloodPressureInterface mInterface) {
        super(address);
        mList = new ArrayList<>();
...
//        TLog.Companion.error("发送++" + ByteUtil.getHexString(CmdUtil.getPlayer("02", Config.BigData.APP_SPECIFY_TEMPERATURE, HexDump.toByteArray(startTime))));
        if (position <= 0)
            return CmdUtil.getFullPackage(CmdUtil.getPlayer("02", Config.BigData.APP_SPECIFY_TEMPERATURE));
        else
            return ByteUtil.hexStringToByte("");
    }
...
//        TLog.Companion.error("getResult++" + getResult);
        if (result[8] == Config.Expand.COMMAND && result[9] == Config.Expand.DEVICE_ACK) {
            switch (result[10]) {
                ...
                case 0x02:
                case 0x03:
                    BleWrite.writeEffectiveBloodPressureCall(mInterface, position,true);
                    isFinishFlag = true;
                    break;
                case 0x04:
...
//                    ShowToast.INSTANCE.showToastLong("展示异常温度数据+"+temperature);
            ...
            mInterface.EffectiveBloodPressureCallResult(mList);
            return true;
